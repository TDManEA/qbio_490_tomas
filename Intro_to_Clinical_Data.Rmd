...
##Path to folder "/Users/tomasmanea/desktop/QBIO490/qbio_490_tomas/analysis_data"
...
```{r}
library(TCGAbiolinks) #required library for TCGA analysis
...
```{r}
write.csv(clinic, "/Users/tomasmanea/desktop/QBIO490/qbio_490_tomas/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/tomasmanea/desktop/QBIO490/qbio_490_tomas/analysis_data/brca_clinical_data.csv")
```
...
#65.69 yld = from adding birth_days_to & last_contact_days_to
...
# create plot here
attach(clinic)
plot(birth_days_to, lymph_nodes_examined_count, main="Scatterplot Example",
   xlab="Age", ylab="Number of Lymph Nodes Examined")
```
...
#Quantitative (discrete) with a categorical variable

For an extra challenge, add a plot title, axes titles, and confidence intervals to your plot.

```{r Exercise 4.2}
# create plot here
class(last_contact_days_to)

boxplot(as.numeric(clinic$last_contact_days_to) ~ clinic$gender)
...
```{r}
under30_over60_mask <- ifelse(age_at_diagnosis < 35 | age_at_diagnosis > 60, TRUE, FALSE )
```
...
```{r}
no_middle_clinic <- clinic[under30_over60_mask, "age_at_diagnosis"]
```
...
```{r}
age_NA_mask <- is.na(age_at_diagnosis)
...
```{r}
cleaned_clinic$age_category <- ifelse(cleaned_clinic <= 35, "Young", 
...
```{r}
cleaned_clinic$survival_time <- ifelse(cleaned_clinic$death_days_to == "[Not Applicable]", "Alive", "Dead")
```
...
```{r}
cleaned_clinic$death_event <- cleaned_clinic$death_event <- ifelse(cleaned_clinic$vital_status == "Dead", "T", "F")
```
